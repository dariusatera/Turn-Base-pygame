...
from TomboBattle import battle_tombol
...
def Battle1(screen):
    ...
    pygame.mixer.music.load(resource_path(os.path.join("sound", "Battle.mp3")))
    pygame.mixer.music.play()
    pygame.mixer.music.set_endevent(SOUND_END_EVENT)

    loop_sound_path = resource_path(os.path.join("sound", "Phalanx_Battle_soundtrack.mp3"))
    music_playing = False

    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("Battle & Party")

    # --- Tampilkan WELCOME screen saat Battle.mp3 mulai ---
    screen.fill(BLACK)
    font_loading = pygame.font.Font(resource_path(os.path.join("font", "MrHorror-Regular.otf")), 50)
    loading_text = font_loading.render("WELCOME", True, RED)
    screen.blit(loading_text, (WIDTH // 2 - 100 , HEIGHT // 2 - 20))
    pygame.display.update()

    # --- Tunggu sampai Battle.mp3 selesai ---
    waiting_music = True 
    while waiting_music:
    ...
    # --- Setelah Battle.mp3 selesai, tampilkan animasi cipratan ---
    blood_imgs = [
        pygame.transform.scale(
            pygame.image.load(resource_path(os.path.join("Asset", "cipratan darah.png"))).convert_alpha(),
            (200, 200)  
        ),
        pygame.transform.scale(
            pygame.image.load(resource_path(os.path.join("Asset", "cipratan darah.png"))).convert_alpha(),
    ...
    blood_sound = pygame.mixer.Sound(resource_path(os.path.join("sound", "BloodEffect.mp3")))

    cipratan_index = 0
    cipratan_show_time = pygame.time.get_ticks()
    cipratan_interval = 800  # ms antar cipratan
    animating = True
    all_cipratan_shown = False
    ...
    while animating:
        ...
        if cipratan_index < 3 and now - cipratan_show_time >= cipratan_interval:
            # Selalu tampilkan WALECOME + semua cipratan yang sudah muncul
            screen.fill(BLACK)
            screen.blit(loading_text, (WIDTH // 2 - 100 , HEIGHT // 2 -20))
            for i in range(cipratan_index + 1):
                screen.blit(blood_imgs[i], blood_positions[i])
            pygame.display.update()
            blood_sound.play()
            cipratan_index += 1 # Naikkan indeks cipratan yang sudah ditampilkan
            cipratan_show_time = now

            if cipratan_index == 3:
                all_cipratan_shown = True

        if all_cipratan_shown and not pygame.mixer.get_busy():
            animating = False

    # --- Setelah animasi cipratan, ganti ke screen battle dan play loop_sound_path ---
    ...
    background_img = pygame.image.load(resource_path(os.path.join("Asset", "background for a tur.png"))).convert()
    ...
    boom_scale = 1.4
    boom_effects = []  # untuk menyimpan efek boom yang aktif
    boom_frame_idx = 0
    boom_frame_timer = pygame.time.get_ticks()
    boom_frame_interval = 50  # ms antar frame boom
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if not isinstance(current_turn, Monster):
                if player_action_mode is None and event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    elif pukul_rect.collidepoint(event.pos):
                        print(f"{current_turn.nama} mempersiapkan Basic Attack!")
                        player_action_mode = "pukul"
                        blink = True
                        blink_count = 0
                    ...
                    if isinstance(current_turn, Healer) and player_action_mode == "ulti":
                        healer_ulti_sound = pygame.mixer.Sound(os.path.join("sound", "healersound", "heal.wav"))
                    ...
                    # Healer - Skill (heal) pilih anggota party
                        ...
                        healer_skill_sound = pygame.mixer.Sound(os.path.join("sound", "healersound", "heal.wav"))
                        ...
                        if target_chosen:
                            if player_action_mode == "pukul":
                                ...
                                energy = min(energy + 1, 4)  # Tambah energi setiap pukul
                                # Efek slash jika Slayer
                                if isinstance(current_turn, Slayer):                                   
                                    slayer_atk_sound = pygame.mixer.Sound(os.path.join("sound", "slayersound", "slashpukul.wav"))
                                ...
                                if isinstance(current_turn, Mage):
                                    ...
                                    boom_scale = 1.4
                                    mage_skill_sound = pygame.mixer.Sound(os.path.join("sound", "magesound", "explosion-312361 (mp3cut.net).wav"))
                                    ...
                                    # Efek slash jika Slayer
                                    if isinstance(current_turn, Slayer):
                                        slayer_skill_sound = pygame.mixer.Sound(os.path.join("sound", "slayersound", "skillslash.mp3"))
                                ...
                                if isinstance(current_turn, Mage):
                                    ...
                                    mage_ulti_actor = current_turn  # Actor yang akan melakukan ulti
                                    player_action_mode = None
                                else:
        ...
        # Mage ulti: tunggu sound ulti selesai, lalu mainkan boom
        if mage_ulti_pending:
            if not mage_ulti_sound_played and not pygame.mixer.get_busy():
                boom_sound = pygame.mixer.Sound(os.path.join("sound", "magesound", "explosion-312361 (mp3cut.net).wav"))
                boom_sound.play()
                # Ganti current_turn.Ultimate(monsters) dengan:
                if mage_ulti_actor:
        ...
        if skill_rect.collidepoint(mouse_pos):
            # Hover: tetap merah terang
            skill_display = font_skill.render("Skill", True, RED)
        elif energy >= 1:
            # Energi cukup: merah tua
        ...
        # Gambar tombol aksi sesuai kondisi
        if show_pukul:
        ...
        if player_action_mode is not None: 
            ...
            elif isinstance(current_turn, Healer) and player_action_mode == "ulti":
                
                target_text = font_target.render("Menyembuhkan seluruh party!", True, RED)
                screen.blit(target_text, (WIDTH // 2 - 100, HEIGHT - 50))
            else:
