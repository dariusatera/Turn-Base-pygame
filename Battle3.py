...
def Battle3(screen):
    ...
    music_path = resource_path(os.path.join("sound", "Phalanx_Battle_soundtrack.mp3"))
    ...
    font_path = resource_path(os.path.join("font", "Exquisite Corpse.ttf"))
    font_pukul = pygame.font.Font(font_path, 55)
    font_skill = pygame.font.Font(font_path, 55)
    font_ulti = pygame.font.Font(font_path, 55)
    ...
    # Load gambar untuk monster
    monster1_frames = [
        pygame.transform.scale(
            pygame.image.load(resource_path(os.path.join("Asset", "slime", "slime1.png"))).convert_alpha(), (monster_radius*5, monster_radius*5)
        ),
        pygame.transform.scale(
            pygame.image.load(resource_path(os.path.join("Asset", "slime", "slime2.png"))).convert_alpha(), (monster_radius*5, monster_radius*5)
        ),
    ]
    ...
    monster_frames = [monster1_frames]
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if not isinstance(current_turn, Monster):
                ...
                elif player_action_mode is not None and event.type == pygame.MOUSEBUTTONDOWN: # ini untuk memilih target
                        ...
                        if target_chosen:
                            if player_action_mode == "pukul":
                                ...
                                if isinstance(current_turn, Slayer):
                                    ...
                                    slash_effect["target_idx"] = chosen_index  # untuk menyimpan indeks monster yang diserang
        ...
        if slash_effect["active"]:
            now = pygame.time.get_ticks()
            if now - slash_frame_timer > slash_frame_interval: # cek apakah waktunya untuk mengganti frame
                slash_frame_idx = (slash_frame_idx + 1) % len(slash_img) # ganti 
